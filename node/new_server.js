...
app.get('/set/:by/:arg', (req, res) => {
    if (!(req.params.by == "health" || req.params.by != "func" || req.params.by != "name")){
        res.send(400);
    } else {
        var pin_set = get_by(req.params.by, req.params.arg);
        set_pin_set(pin_set.valid, 0);
        set_pin_set(pin_set.invalid, 1);
        res.send(200);
    }
})
function get_by(by, arg){
    ...
    pin_num.forEach(num => {
        if (pin_conf[num][by] == arg)
            ...
            pin_set.invalid.push(pin_conf[num].pin);
    });
    return pin_set;
}
